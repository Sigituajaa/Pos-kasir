...
async function decreaseStockForOrder(items) {
    ...
    try {
        await runTransaction(db, async (transaction) => {
            ...
            const currentProducts = snap.data().items || [];

            items.forEach(item => {
                const lookupId = item.productId ?? item.id; // item keranjang varian pakai productId ke produk aslinya
                const idx = currentProducts.findIndex(p => p.id === lookupId);
                if (idx !== -1 && currentProducts[idx].stock != null) {
                    currentProducts[idx].stock = Math.max(0, currentProducts[idx].stock - item.qty);
                }
            });
...
function showAttendanceOverlay(type, settings) {
    ...
    setDashboardLocked(true); // kunci sungguhan: dashboard di baliknya benar-benar tidak bisa diklik
    lucide.createIcons();
}
...
// Kunci sungguhan (bukan cuma visual): saat overlay/gerbang tunggu aktif, elemen di baliknya
// dikasih pointer-events:none sekaligus inert, jadi walau ada bug CSS/z-index, dashboard TETAP
// tidak bisa disentuh sampai terkunci ini dilepas.
...
function saveAttendanceRecord(type, employee) {
    ...
    const field = type === 'masuk'
        ? { masukTime: now, masukBy: employee ? employee.name : null }
