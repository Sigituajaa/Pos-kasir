...
function openSalesReport() {
    renderSalesReportFilterOptions();
    renderSalesReport();
...
// Isi dropdown filter user di modal Sales Hari Ini — dipanggil tiap kali modal dibuka supaya
// daftar karyawan selalu sinkron.
function renderSalesReportFilterOptions() {
...
function renderSalesReport() {
    const filterSelect = document.getElementById('sales-report-employee-filter');
    const empId = filterSelect ? filterSelect.value : '';
    ...
    if (todaysOrders.length === 0) {
        body.innerHTML = `<tr><td colspan="4" class="text-center p-8 text-slate-400 text-xs">Belum ada transaksi ${empId ? 'untuk karyawan ini ' : ''}hari ini</td></tr>`;
        return;
    }

    // Tidak ada lagi header nama+subtotal per grup di sini — total & jumlah transaksi per user
    // cukup dilihat lewat dropdown filter di atas (sudah tercermin di kotak ringkasan di atas).
