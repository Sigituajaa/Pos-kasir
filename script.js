...
function renderAdminTools() {
    ...
    // Total Transaksi (dengan filter per karyawan / tanggal)
    renderTotalTransaksiFilterOptions();
    applyTotalTransaksiFilter();
}

// --- TOTAL TRANSAKSI (dengan filter per karyawan & tanggal) ---
function renderTotalTransaksiFilterOptions() {
...
function getFilteredOrderHistory() {
    ...
    const dateVal = dateInput ? dateInput.value : ''; // format YYYY-MM-DD dari <input type="date">

    return orderHistory.filter(o => {
        if (empId && o.employeeId !== empId) return false;
        if (dateVal && o.timestamp && getTodayDateStr(new Date(o.timestamp)) !== dateVal) return false;
        return true;
    });
}

function applyTotalTransaksiFilter() {
...
function resetTotalTransaksiFilter() {
    const empSelect = document.getElementById('report-filter-employee');
    const dateInput = document.getElementById('report-filter-date');
...
// --- JADWAL KERJA KARYAWAN (KALENDER BULANAN + JAM KERJA PER KARYAWAN) ---
// scheduleCache: { 'YYYY-MM-DD': { date, employees: [ { id, jamMulai, jamSelesai } ] } }
let scheduleCache = {};
let scheduleViewDate = new Date(); // bulan yang sedang ditampilkan di kalender
let scheduleSelectedEmployeeId = null; // karyawan yang lagi dipilih buat diatur jadwalnya
let scheduleSelectedDateStr = null; // tanggal yang lagi diedit jam kerjanya
...
// Chip nama karyawan di atas kalender — klik satu buat "terpilih" (di-highlight), lalu tap
// tanggal di kalender buat atur jam kerja karyawan tsb di tanggal itu.
function renderScheduleEmployeeChips() {
    const container = document.getElementById('schedule-employee-chips');
    if (!container) return;
    container.innerHTML = employeesCache.map(emp => `
        <button onclick="selectScheduleEmployee('${emp.id}')" class="schedule-emp-chip ${emp.id === scheduleSelectedEmployeeId ? 'active' : ''}">${emp.name}</button>
    `).join('') || '<p class="text-xs text-slate-400">Tambahkan karyawan dulu di atas.</p>';
}

function selectScheduleEmployee(empId) {
    scheduleSelectedEmployeeId = empId;
    scheduleSelectedDateStr = null; // reset tanggal yang lagi diedit, biar tidak salah karyawan
    const editor = document.getElementById('schedule-jam-editor');
...
function renderScheduleCalendar() {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
        ...
        const employeesOnDate = (entry && entry.employees) || [];
        const names = employeesOnDate.map(e => (employeesCache.find(emp => emp.id === e.id) || {}).name).filter(Boolean);
        const isToday = dateStr === todayStr;
        const isSelectedDate = dateStr === scheduleSelectedDateStr;
        const isSelectedEmpScheduled = scheduleSelectedEmployeeId && employeesOnDate.some(e => e.id === scheduleSelectedEmployeeId);
        const cellClass = isSelectedDate ? 'bg-pink-100 ring-2 ring-pink-500' : isToday ? 'bg-indigo-100 ring-1 ring-indigo-400' : isSelectedEmpScheduled ? 'bg-emerald-50 ring-1 ring-emerald-300' : 'bg-slate-50 hover:bg-slate-100';

        cells += `
        <button onclick="selectScheduleDate('${dateStr}')" class="aspect-square rounded-lg p-1 flex flex-col items-center justify-start text-left ${cellClass} transition overflow-hidden">
...
// Diklik dari kalender. Butuh karyawan sudah terpilih dulu (lewat chip) sebelum bisa atur jam kerja.
function selectScheduleDate(dateStr) {
    if (!scheduleSelectedEmployeeId) {
        showToast('Pilih nama karyawan dulu di atas kalender.', 'warn');
        return;
    }
    scheduleSelectedDateStr = dateStr;
    const entry = scheduleCache[dateStr];
    const empEntry = entry && entry.employees ? entry.employees.find(e => e.id === scheduleSelectedEmployeeId) : null;
    const emp = employeesCache.find(e => e.id === scheduleSelectedEmployeeId);
    const dateObj = new Date(dateStr + 'T00:00:00');

    document.getElementById('schedule-jam-editor-title').innerText = `${emp ? emp.name : ''} — ${dateObj.toLocaleDateString('id-ID', { weekday: 'long', day: 'numeric', month: 'long' })}`;
    document.getElementById('schedule-jam-mulai').value = empEntry ? (empEntry.jamMulai || '') : '';
    document.getElementById('schedule-jam-selesai').value = empEntry ? (empEntry.jamSelesai || '') : '';
    document.getElementById('schedule-jam-remove-btn').classList.toggle('hidden', !empEntry);

    document.getElementById('schedule-jam-editor').classList.remove('hidden');
    renderScheduleCalendar(); // supaya tanggal yang lagi dipilih ke-highlight
    lucide.createIcons();
}

function saveScheduleJamKerja() {
    if (!scheduleSelectedEmployeeId || !scheduleSelectedDateStr) return;
    if (!window.FB || !window.FB.ready) return showToast('Belum terhubung ke database.', 'warn');

    const jamMulai = document.getElementById('schedule-jam-mulai').value;
    const jamSelesai = document.getElementById('schedule-jam-selesai').value;
    if (!jamMulai || !jamSelesai) return alert('Isi jam mulai & selesai kerja!');

    const { db, doc, setDoc } = window.FB;
    const dateStr = scheduleSelectedDateStr;
    const existing = (scheduleCache[dateStr] && scheduleCache[dateStr].employees) || [];
    const updatedEmployees = [...existing.filter(e => e.id !== scheduleSelectedEmployeeId), { id: scheduleSelectedEmployeeId, jamMulai, jamSelesai }];
...
function removeScheduleJamKerja() {
    if (!scheduleSelectedEmployeeId || !scheduleSelectedDateStr) return;
    if (!window.FB || !window.FB.ready) return showToast('Belum terhubung ke database.', 'warn');
    ...
    const dateStr = scheduleSelectedDateStr;
    const existing = (scheduleCache[dateStr] && scheduleCache[dateStr].employees) || [];
    const remaining = existing.filter(e => e.id !== scheduleSelectedEmployeeId);
    ...
    savePromise.then(() => {
        ...
        document.getElementById('schedule-jam-editor').classList.add('hidden');
        scheduleSelectedDateStr = null;
        renderScheduleCalendar();
    }).catch((err) => {
