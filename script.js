...
function getAttendanceSettings() {
    // Jam masuk sudah tidak lagi pakai jendela global — sekarang ditentukan per karyawan lewat
    // tabel Jadwal Kerja Karyawan (lihat getTodayScheduleEntryForCurrentKasir). Cuma jendela
    // absen PULANG yang masih pakai pengaturan global di sini.
...
function saveAttendanceSettings() {
    ...
    if (!settings.keluarStart || !settings.keluarEnd) {
        return alert('Jam jendela absen pulang wajib diisi!');
    }
...
// Absen PULANG tetap terikat jendela jam seperti sebelumnya (lihat checkAttendanceOverlay).
...
function saveAttendanceRecord(type, employee) {
    ...
    const field = type === 'masuk'
        ...
        : { keluarTime: now, keluarBy: employee ? employee.name : null, keluarById: employee ? employee.id : null };
