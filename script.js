...
function getAttendanceSettings() {
    const saved = JSON.parse(localStorage.getItem('pos_attendance_settings')) || {};
    const settings = {
        kasirName: saved.kasirName || '',
        masukTime: saved.masukTime || '14:00',
        masukTolerance: saved.masukTolerance != null ? clampTolerance(saved.masukTolerance) : 60,
        keluarTime: saved.keluarTime || '22:00',
        keluarTolerance: saved.keluarTolerance != null ? clampTolerance(saved.keluarTolerance) : 60
    };
    const masukWin = computeAttendanceWindow(settings.masukTime, settings.masukTolerance);
    const keluarWin = computeAttendanceWindow(settings.keluarTime, settings.keluarTolerance);
    settings.masukStart = masukWin.start;
    settings.masukEnd = masukWin.end;
    settings.keluarStart = keluarWin.start;
    settings.keluarEnd = keluarWin.end;
    return settings;
}

function saveAttendanceSettings() {
    const masukTime = document.getElementById('att-masuk-time').value;
    const keluarTime = document.getElementById('att-keluar-time').value;
    const masukTolerance = clampTolerance(document.getElementById('att-masuk-tol').value);
    const keluarTolerance = clampTolerance(document.getElementById('att-keluar-tol').value);

    if (!masukTime || !keluarTime) {
        return alert('Jam pokok absen masuk & pulang wajib diisi!');
    }

    const settings = {
        kasirName: document.getElementById('att-kasir-name').value.trim(),
        masukTime, masukTolerance, keluarTime, keluarTolerance
    };
...
// Tampilkan pratinjau jendela absen (dihitung real-time) di bawah form pengaturan Admin
function updateAttendanceWindowPreview() {
    const mtEl = document.getElementById('att-masuk-time');
    const mTolEl = document.getElementById('att-masuk-tol');
    const ktEl = document.getElementById('att-keluar-time');
    const kTolEl = document.getElementById('att-keluar-tol');
    const masukPrev = document.getElementById('att-masuk-preview');
    const keluarPrev = document.getElementById('att-keluar-preview');

    if (mtEl && masukPrev) {
        if (mtEl.value) {
            const w = computeAttendanceWindow(mtEl.value, clampTolerance(mTolEl.value));
            masukPrev.innerText = `Jendela absen: ${w.start} - ${w.end}`;
        } else {
            masukPrev.innerText = 'Jendela absen: -';
        }
    }
    if (ktEl && keluarPrev) {
        if (ktEl.value) {
            const w = computeAttendanceWindow(ktEl.value, clampTolerance(kTolEl.value));
            keluarPrev.innerText = `Jendela absen: ${w.start} - ${w.end}`;
        } else {
            keluarPrev.innerText = 'Jendela absen: -';
        }
    }
}
...
function loadAttendanceSettingsToForm() {
    ...
    if (mtEl) mtEl.value = s.masukTime;
    if (mTolEl) mTolEl.value = s.masukTolerance;
    if (ktEl) ktEl.value = s.keluarTime;
    if (kTolEl) kTolEl.value = s.keluarTolerance;
    updateAttendanceWindowPreview();
    renderAttendanceQR();
...
// supaya Admin bisa akses menu admin (ambil data absen karyawan/kasir) tanpa lewat kasir dulu.
...
function recordAttendanceManual(type) {
    ...
    // maks 120 menit). Di luar jendela ini, absen ditolak & kasir harus menunggu jendela berikutnya.
    const settings = getAttendanceSettings();
    const now = new Date();
