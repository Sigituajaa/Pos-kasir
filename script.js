...
// Paket seperti "Cireng Kuah Keju" tidak punya stock sendiri (dianggap dirakit dari cireng satuan),
// jadi saat item punya variantSelections, yang berkurang adalah stock tiap cireng satuan yang dipilih
// (dikali qty item di keranjang), BUKAN stock produk paketnya sendiri.
    ...
    items.forEach(item => {
        if (item.variantSelections && item.variantSelections.length > 0) {
            item.variantSelections.forEach(sel => {
                const idx = products.findIndex(p => p.id === sel.id);
                if (idx !== -1 && products[idx].stock != null) {
                    products[idx].stock = Math.max(0, products[idx].stock - (sel.qty * item.qty));
                    changed = true;
                }
            });
            return;
...
function adjustVariantQty(optionId, delta) {
    ...
    // Tidak boleh pilih lebih banyak dari stock cireng satuan yang tersedia (kalau stock dilacak)
    if (delta > 0) {
        const option = state.options.find(o => o.id === optionId);
        if (option && option.stock != null && next > option.stock) {
            showToast(`Stock ${option.name} tinggal ${option.stock}!`, 'warn');
            return;
        }
    }
...
function renderVariantOptions() {
    ...
    document.getElementById('variant-options').innerHTML = state.options.map(o => {
        const isOutOfStock = o.stock != null && o.stock <= 0;
        const atStockLimit = o.stock != null && (state.selections[o.id] || 0) >= o.stock;
        return `
        <div class="flex items-center justify-between bg-white p-4 rounded-2xl border border-slate-100">
            <div>
                ...
                ${o.stock != null ? `<p class="text-[10px] ${isOutOfStock ? 'text-red-500 font-bold' : 'text-slate-400'} mt-0.5">${isOutOfStock ? 'Stock habis' : `Sisa ${o.stock}`}</p>` : ''}
            </div>
            <div class="flex items-center gap-3 bg-slate-50 p-1 rounded-xl font-bold">
                ...
                <button onclick="adjustVariantQty(${o.id}, 1)" ${atStockLimit ? 'disabled' : ''} class="qty-btn w-8 h-8 ${atStockLimit ? 'text-slate-200' : 'text-slate-400'}">+</button>
            </div>
        </div>`;
    }).join('');
...
function confirmVariantSelection() {
    ...
    // Simpan id produk asli tiap isi yang dipilih, supaya saat checkout stock CIRENG SATUANNYA
    // yang berkurang (paket sendiri tidak dilacak stocknya, cuma "merakit" dari stock satuan).
    ...
    cart.push({
        id: `v_${Date.now()}`,
        productId: state.product.id, // referensi ke produk paket asli (harga & tampilan di keranjang)
...
let activeProofId = null; // proof yang sedang ditampilkan di modal aksi kirim (WhatsApp)
...
// Tabel di layar Admin hanya menampilkan 7 hari terakhir supaya ringkas;
// laporan lengkap tetap bisa diunduh utuh lewat tombol Download PDF (lihat downloadAttendancePDF()).
function renderAttendanceHistory() {
    ...
    body.innerHTML = last7Days.map(r => {
...
// Download SELURUH riwayat absensi (tidak dibatasi 7 hari) dalam format PDF
function downloadAttendancePDF() {
    const log = getAttendanceLog().slice().sort((a, b) => b.date.localeCompare(a.date));
    if (log.length === 0) {
        showToast('Belum ada riwayat absensi untuk diunduh.', 'warn');
        return;
    }

    const { jsPDF } = window.jspdf;
    const doc = new jsPDF();
    doc.text(`Riwayat Absensi Karyawan - ${STORE_NAME}`, 14, 12);

    const kasirName = getKasirName();
    if (kasirName) doc.text(`Nama Kasir: ${kasirName}`, 14, 19);

    const data = log.map(r => {
        const masukStr = r.masukTime ? new Date(r.masukTime).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' }) : '-';
        const keluarStr = r.keluarTime ? new Date(r.keluarTime).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' }) : '-';
        let durasi = '-';
        if (r.masukTime && r.keluarTime) durasi = formatDurationHM(new Date(r.keluarTime) - new Date(r.masukTime));
        const tanggalFormatted = new Date(r.date + 'T00:00:00').toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' });
        return [tanggalFormatted, masukStr, keluarStr, durasi];
    });

    doc.autoTable({
        head: [['Tanggal', 'Jam Masuk', 'Jam Pulang', 'Durasi Kerja']],
        body: data,
        startY: kasirName ? 24 : 18
    });
